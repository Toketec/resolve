...
version: "1.0"
date: "2026-06-28"
scope: "orchestrator调度 → 6选3 → 并行推理 → 加权共识"
---

# ✅ Spec 3 — AI Oracle Reasoning Pipeline 验收方案

> **用途**: AI 自检 + 人工验证，确保真实 LLM 调用链（Orchestrator → 6选3 → 并行推理 → 加权共识）可正确执行、可稳定复现、可容错回退。
...
console.log('selectAgents:', typeof ai.selectAgents);
"
...
| # | 操作步骤 | 预期结果 | 加分项 |
|:-:|----------|----------|:------:|
| 3.1 | `ANTHROPIC_API_KEY=sk-xxx pnpm --filter @resolve/ai exec tsx -e "import {resolveMarket} from './src/index'; resolveMarket({id:'btc-150k-2026',question:'Will BTC close above $150K by Dec 31, 2026?'}).then(console.log)"` | 返回 JSON 对象，包含 `status` / `consensus_score` / `orchestrator` / `votes` 字段 | — |
| 3.1b | 检查返回的 `votes` 数组 | 数组长度为 3，对应 orchestrator 选中的 3 个 Agent | — |
| 3.1c | 检查 `orchestrator` 字段 | `{ selected: string[], reasoning: string }`，reasoning 可读、可展示 | 🏆 |
...
| 3.2c | 检查 evidence 的 `kind` 多样性 | 3 个 Agent 的 evidence kind 类型不同（e.g., `price`, `news`, `onchain`） | 🏆 |
...
| 3.4d | 3 个 Agent 的 outcome 是否全部相同 | 允许全 YES（若市场证据强烈偏多），但至少 2 家 confidence 存在差异 | 🏆 |
...
| 3.6c | 两次调用的 `orchestrator.selected` 一致 | 选中的 3 个 Agent ID 完全相同 | 🏆 |
...
| E3 | Orchestrator 选择失败（API 错误） | 回退到默认 3 个 Agent（BULL-1/BEAR-1/NEUT-1） | 不影响后续推理 |
