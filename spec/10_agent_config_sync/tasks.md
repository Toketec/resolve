...
| ID | 任务 | 文件 | 优先级 | 状态 |
|:--:|------|------|:-----:|:----:|
| T01 | 新建 `agents.ts`：`AgentMeta` + `AGENT_META`（SQL 镜像兜底）+ `KIND_FROM_ROLE` | `packages/shared/src/agents.ts` | P0 | ⬜ |
...
| T05 | DB seeding 脚本：从 `AGENT_META` 导入 → upsert，不再重复定义 | `packages/db/scripts/seed-agents.ts` | P1 | ⬜ |
| T06 | `pnpm typecheck && pnpm build` | — | P0 | ⬜ |

## 完成条件

- [ ] `packages/shared/src/agents.ts` — 6 个 Agent 配置完整（SQL 镜像兜底）
...
```
SQL migration  ←── 唯一编辑点 (规范数据)
      │
      ▼
agents.ts  ←── 兜底镜像 (mirror SQL, 唯一定义)
  ├── mock/agents.ts  → import
  ├── mappers.ts      → import
  └── seed-agents.ts  → import → upsert
```
