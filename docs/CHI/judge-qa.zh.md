...
|:----|:---:|--------|:----------:|
| ⚡ **BULL-1** (交易所) | 1 | HTX 价格/订单簿 → 偏多分析 | ✅ 真实调用 |
| ⚡ **BULL-2** (技术) | 1 | TEE/L2 技术面 → 偏多补充 | ✅ 真实调用 |
| ⚡ **BEAR-1** (媒体) | 1 | 新闻情绪/监管 → 偏空分析 | ✅ 真实调用 |
| ⚡ **BEAR-2** (监管) | 1 | 全球监管政策 → 偏空补充 | ✅ 真实调用 |
...
### Q: 为什么 6 个 Agent 不是逐个出现而是同时出现？

**因为 Promise.all 并行调用。** 6 个 Agent 在同一时间收到证据、各自调用 LLM，谁快谁先返回。但 UI 上通过动画控制了**展示顺序**——votes 一条一条按序浮现（约 1-1.5 秒间隔），制造"逐个推理"的视觉效果，同时不增加实际总耗时。

- 实际耗时：6 次并行 ≈ 5-6 秒（最慢的那个 Agent 决定总时长）
- UI 动画：votes 逐条展示 ≈ 6-9 秒
- Demo 效果：观众看到一条条 Agent 投票浮现，感觉 AI 在"逐步思考"
