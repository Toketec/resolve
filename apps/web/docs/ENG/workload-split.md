...
Status: **locked** after grill Q1–Q12. This is the *current* split, re-shaped from the original — notably, chain-adjacent work moved off Dev A where it isn't wallet/value work, and the pitch + integration seam got explicit owners.
...
- **Real agent resolution** — replace the hardcoded `buildConsensus()` in [lib/mock/markets.ts](../lib/mock/markets.ts) (currently returns a fixed `0.62` / `0.92`). Feed market question + criteria → real Claude calls → each agent returns `{ outcome, confidence, evidence }`.
...
- **Consensus math** — the aggregation function, typed against `AgentVote` / `AIConsensus` in [lib/types.ts](../lib/types.ts).
...
- **Buy UI + API** — the [trade-panel.tsx](../components/trade-panel.tsx) flow and its route; A supplies only the signing call.
- **The live demo shot** — C drives it: the staged vote reveal, the on-demand expiry trigger, the continuous flow. (See G4, G5 in [open-questions.md](./open-questions.md).)
...
[lib/types.ts](../lib/types.ts) is the shared truth. Nobody edits types without telling the other two. Full signatures in [api-contracts.md](./api-contracts.md).
