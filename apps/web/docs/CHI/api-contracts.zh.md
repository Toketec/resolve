...
共享类型词汇已存在于 [lib/types.ts](../lib/types.ts) —— `Market`、`AIConsensus`、`AgentVote`、`Evidence`、`Position`、`Outcome`。**不通知另外两位开发者，禁止改动这些类型。**
...
- 输出：完整的 `AIConsensus`（`status`、`outcome`、`confidence`、`threshold`、`votes[]`、时间戳）——正是 [ConsensusMeter](../components/consensus-meter.tsx) 和[详情页](../app/markets/[slug]/page.tsx)已经在渲染的形状。
- 内部 B 对精选证据集做 N 次真实 Claude 调用（N = 3，待 [G6](./open-questions.zh.md) 确认），然后跑共识数学。
...
async function buyShares(args: {
  marketId: string;
  side: Outcome;          // "YES" | "NO"
  amount: number;         // 单位 USDD（见 G3）
  walletAddress: string;  // 已连接的 TronLink 钱包（见 G7）
...
- `buyShares` —— C 处理表单/路由（[trade-panel.tsx](../components/trade-panel.tsx)）；A 只处理签名或转移价值的那几行。
...
与其说是切片间的代码接缝，不如说是一次规格交接。依据 [G6/Q6](./open-questions.zh.md)：
